"""
test_run_phase4b.py — Smoke test for the Phase 4B comparison runner.
...
tests/test_run_phase4.py's pattern for the prior runner.
"""
...
def _params() -> dict:
    return {
        ...
        "ml_signals": {
            ...
            "random_forest": {"n_estimators": 20, "max_depth": 3, "random_state": 0},
            "xgboost": {"n_estimators": 20, "max_depth": 3, "random_state": 0},
        },
...
def _patch(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(run_phase4b, "load_params", lambda: _params())
    monkeypatch.setenv("MLFLOW_TRACKING_URI", f"sqlite:///{tmp_path / 'mlflow_test.db'}")


def test_run_phase4b_writes_results_with_full_trial_pool(tmp_path, monkeypatch):
    _write_gold_snapshot(tmp_path)
    _patch(monkeypatch, tmp_path)

    all_results = run_phase4b.run_phase4b()
    ...
    for universe_results in all_results.values():
        # 7 existing (Phase 2 + Phase 4) + 2 new F7 strategies.
    ...
    for entry in output.values():
        ...
        assert "beats_phase4_hurdle" in entry
        assert "is_phase4b_strategy" in entry
        # No stored phase4_results.json under tmp_path -> comparison is None.
        assert entry["beats_phase4_hurdle"] is None


def test_run_phase4b_compares_against_a_stored_hurdle(tmp_path, monkeypatch):
    _write_gold_snapshot(tmp_path)
    hurdle = {
