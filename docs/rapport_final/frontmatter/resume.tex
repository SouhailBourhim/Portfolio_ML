...
\integriteReplis{} des \integriteAjustements{} ajustements — \integriteStrategies{}
stratégies évaluées, \integriteDates{} dates de rééquilibrage — ont emprunté un
repli d'estimateur~: sur ces rééquilibrages, le résultat provient d'un estimateur
de substitution et non du modèle que l'étiquette désigne. Le décompte est
généré depuis les artefacts, versionné, et confronté à un second artefact
indépendant. Après correction White/SPA pour les 240 configurations
...
prêt mais non actif, couverts par \textbf{plus de 780~tests automatisés}. Les corrections
...
levels are therefore not comparable across universes. On the released snapshot
\integriteReplis{} of \integriteAjustements{} fits — \integriteStrategies{} evaluated
strategies, \integriteDates{} rebalance dates — used an estimator fallback: on those
rebalances the result came from a substitute estimator rather than the model its
label names. The count is generated from the artifacts, versioned, and cross-checked
against an independent second artifact. After White/SPA
...
that is ready but deliberately not live, covered by \textbf{more than 780 automated
