...
% =============================================================================
\section{Deux univers, et une frontière de données canonique}
% =============================================================================
...
La chaîne canonique rencontre une frontière claire~: \textbf{aucune source
gratuite, contractualisée et reproductible dans ce pipeline ne fournit l'historique
complet de la Bourse de Casablanca avant la mi-2021}. Le portefeuille complet
publié ne peut donc pas être étudié sur le krach de 2020, ni sur la crise de
2008. Plutôt que de dissimuler cette limite, nous avons adopté une \textbf{double
base de test} (tableau~\ref{tab:univers})~: toute stratégie est évaluée sur les
deux univers, et les deux résultats sont publiés côte à côte.
...
Cette frontière ne signifie pas que les données marocaines antérieures à 2021
n'existent pas. Une étude séparée a constitué un univers BVC profond de 12 actions
sur 2005--2024 (56\,184 lignes de panel, soit environ cinq fois le volume de
\texttt{full\_2021}) et couvrant 2008. Elle a amélioré le coefficient
d'information des signaux RF/XGB d'un facteur deux à quatre, sans établir un
avantage de portefeuille net de coûts. Cet historique reste hors de la release~:
ses prix bruts doivent encore être contractualisés, rapprochés de la période de
recouvrement récente et ajustés des dividendes et opérations sur titres. C'est une
limite de gouvernance des données et d'intégration, non une absence de données.
...
\begin{resultatbox}{Effet de la correction — et la suite de la chaîne}
% HISTORIQUE. Les deux premieres valeurs sont SUPERSEDED : elles sont conservees
% pour montrer la revision successive, jamais comme resultat courant. La chaine
% est deroulee jusqu'au chiffre actuel pour qu'aucune valeur ancienne ne puisse
% etre confondue avec lui.
Estimation initiale~: $+14{,}3$~\%. Après correction des dividendes~:
$+6{,}2$~\%. \textbf{Ces deux valeurs sont désormais \emph{superseded}.} Après
conversion du portefeuille en dirhams aux taux de référence officiels de Bank
Al-Maghrib, l'écart ponctuel devient \textbf{$-10{,}47$~\%}. Ces écarts sont
descriptifs~: ils ne constituent pas un test pairé.

Un second résultat, présenté depuis le début du projet, ne survit pas non plus~:
...
Le dépôt compte \textbf{plus de 780 tests}, exécutés hors ligne
...
\begin{figure}[htbp]
  \centering
  \fbox{\includegraphics[width=0.95\textwidth]{assets/figures/dagster_assets.png}}
...
C'est le registre qui permet, six semaines après, de répondre à la question
« quel Sharpe publié, sur quelles données et avec quels réglages~? » sans relancer
un calcul. Les valeurs de performance courantes ne sont jamais lues dans une
capture d'écran~: elles proviennent des artefacts Gold versionnés du
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{3.3cm}p{9.2cm}@{}}
    ...
    pytest & Plus de 780 tests, hors ligne, exécutés à chaque modification. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item L'univers BVC \emph{canonique} commence en 2021. Un historique marocain
        profond a été testé séparément~: il améliore la prédiction sans établir
        d'avantage de portefeuille, et reste hors release tant que ses sources et
        ses ajustements ne sont pas contractualisés. La \textbf{double base de
        test} publie les désaccords entre univers au lieu de les masquer.
