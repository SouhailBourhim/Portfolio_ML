...
avertissement. Ce chemin est testé et instrumenté ; son taux d'activation sur
l'instantané de release est rapporté avec les résultats au
chapitre~\ref{chap:resultats}, plutôt que déduit d'une observation historique.
\end{remarquebox}
...
\begin{figure}[htbp]
  ...
  de savoir si les amas rouges sont associés à ces bandes est traitée comme une
  analyse exploratoire au chapitre~\ref{chap:resultats}.}
  \label{fig:regimes}
\end{figure}
...
\begin{definitionbox}{Purge, embargo, et sélection strictement antérieure}
...
l'horizon chevauche le bloc de validation~; l'\emph{embargo} écarte en outre une
bande de journées juste après ce bloc.

Ces deux mécanismes ne suffisent pas. La validation croisée purgée classique
entraîne le modèle sur \emph{toutes} les dates hors de la bande — y compris
\textbf{postérieures} à la validation. La \emph{sélection strictement antérieure}
ajoute la contrainte manquante~: l'entraînement précède toujours la validation,
et chaque pli vérifie
...
Les \emph{hyperparamètres des modèles} sont donc choisis par \textbf{validation
strictement antérieure} (folds \emph{forward-only} : l'entraînement précède
toujours la validation, séparé d'une purge et d'un embargo), au coefficient
d'information — rapporté \emph{par pli} et non seulement en moyenne, une moyenne
identique pouvant recouvrir cinq plis stables ou un seul pli dominant. Les \emph{leviers de portefeuille}
...
\begin{itemize}
  ...
  \item L'évaluation traite le problème P4 comme un problème à part entière~:
        rejeu du temps, sélection des réglages \textbf{strictement antérieure}
        (purge et embargo à la frontière, l'entraînement précédant toujours la
        validation), segment de test gelé, \textbf{comparaisons pairées} par
        bootstrap à blocs — le test de la \emph{différence}, que des intervalles
        marginaux ne fournissent pas — et correction pour le nombre de
        configurations essayées, compté automatiquement et non déclaré.
