...
déflaté est 0,6705 sur 198 configurations. Aucun adjectif n'est accolé à cette
...
Le décompte publié figure dans l'énoncé « Intégrité des modèles » du chapitre
\emph{Faits publiés} et dans le tableau~\ref{tab:integrite} : il est produit à
partir de \texttt{data/gold/fit\_report\_summary.json} et n'est jamais saisi
ici. Cette mesure ne signifie pas qu'un modèle ne peut jamais échouer : les
chemins de repli sont du code vivant et testé, et un autre snapshot peut les
emprunter. Sa portée est exactement l'ensemble compté.

Un décompte que seul l'artefact concerné peut observer ne vaut rien : le
2026-08-10, le résumé de télémétrie annonçait zéro repli alors que
\texttt{dashboard\_regime.parquet} enregistrait six rééquilibrages sans
convergence, parce qu'aucun point d'instrumentation n'existait sur les chemins
SLSQP et HMM. Le compte est désormais confronté à ce second artefact par
\texttt{TestFallbackCountsAgree}. Les caches réémettent par ailleurs la
télémétrie à chaque lecture, afin qu'un cache chaud ne fasse pas
artificiellement diminuer le taux de repli.
