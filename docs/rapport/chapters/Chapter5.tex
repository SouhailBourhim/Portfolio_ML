...
dans \texttt{phase5\_validation\_protocol.json} plutôt qu'affirmée ici.
...
\textbf{Aucune preuve de surperformance.} Sur les huit comparaisons pairées, tous
les intervalles contiennent zéro et toutes les p-values dépassent 0,05 ; la plus
proche du seuil est RF contre l'équipondéré sur l'univers ETF ($p = 0{,}066$).

Un cas mérite d'être isolé, car il est exactement ce que ce test sert à révéler.
Sur \texttt{full\_2021}, le signal XGB calibré obtient 1,308 contre 1,213 pour la
stratégie à régimes : en \emph{estimation ponctuelle}, il « bat » la référence, et
l'artefact l'enregistre comme tel. Le test pairé donne pourtant
$\Delta = +0{,}096$, IC $[-0{,}315;\ +0{,}541]$, $p = 0{,}361$. Le classement
