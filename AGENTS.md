...
> pre-specified primary benchmark. Telemetry reports **6 fallbacks in 1,184 fits**
> on the released snapshot — all six are `regime_conditional` in its
> `min_regime_train_days` warm-up, where the allocation is produced entirely by
> `min_variance_lw` (6.2% of that comparator's `full_2021` OOS days). Any earlier
> "0 fallbacks in 1,188 fits" wording is RETRACTED: it was not a measurement of
> zero but the absence of a measurement, because `telemetry.record` had no call
> site on the SLSQP or HMM degradation paths and `telemetry.summarize` returns
> `ok` when given no records. The count is now generated by
> `release_facts.model_integrity_statement` and cross-checked against
> `dashboard_regime.parquet`. Model cards and exact explanation artefacts are
